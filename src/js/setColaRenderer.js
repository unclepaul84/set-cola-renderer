(function (exports) {
  ...
  var rendererClass = function (options) {
    ...
    renderer.draw = function (layout) {
      ...
      // Clear the old graph
      d3.select('.' + renderer.options['targetsvgclass']).selectAll('g').remove();
    ...
    renderer.showLayoutBoundaries = function () {
      ...
      var width = d3.select('svg').style('width').replace('px', ''),
        height = d3.select('svg').style('height').replace('px', '');
      var padding = 50;
      var boundary_line = d3.select('.' + renderer.options['targetsvgclass']).selectAll('.boundary')
        .data(boundaries)
        .attr('x1', function (d) {
      ...
      var boundary_text = d3.select('.' + renderer.options['targetsvgclass']).selectAll('.boundary-text')
        .data(boundaries)
    ...
    function averageNodePosition() {
      ...
      var averageX = sumX / renderer.setcola.nodes.length;
      var averageY = sumY / renderer.setcola.nodes.length;
  ...
  exports.setColaRenderer = rendererClass;
